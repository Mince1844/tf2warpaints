...
::Warpaints <- {
	...
	PAINTS = {
		...
		"sandwich diner": 433, "beachy boy": 434, "army guns": 435, "taxi cabbed": 436, "ocean mapped": 437,
		"krak coated": 438, "team union": 439, "sideshow": 440, "storage war": 441, "die'n dasher": 442,
	},
	...
	{
		local pdata = ValidatePlayerData(player);
		if (!pdata) return;
		...
		local lower_32_bits = (seed != null) ? seed & 0xFFFFFFFF : 0;
		local upper_32_bits = (seed != null) ? (seed >> 32) & 0xFFFFFFFF : 0;
		...
		local data = [
			...
			{val=seed, name="seed", cast2f=true, attrs={"custom_paintkit_seed_lo": lower_32_bits, "custom_paintkit_seed_hi": upper_32_bits}},
		];
		
		// Apply attributes
		foreach (table in data)
		{
			table.val = (table.val != null) ? table.val : pdata[table.name];
			if (table.val != null)
			{
				pdata[table.name] = table.val;
	...
	{
		local t1 = type(object);
		if (t1 == "array" || t1 == "table")
		{
			local copy = (t1 == "array") ? [] : {};
	...
	{
		if (!player || !(player instanceof CTFPlayer) || !player.IsValid())
		{
		...
		if (!(player in PlayerData))
		{
			pdata = DeepCopy(DEFAULT_PLAYER_SCOPE);
		...
		if (pdata.self != player) pdata.self = player;

		return PlayerData[player];
	},
	
	function PlayerThink() {
		local pdata = Warpaints.ValidatePlayerData(self);
		if (!pdata) return;

		local buttons = NetProps.GetPropInt(self, "m_nButtons");
		local buttons_changed = pdata.buttons_last ^ buttons;
		local buttons_pressed = buttons_changed & buttons;
		local buttons_released = buttons_changed & (~buttons);
		
		if (buttons_pressed & IN_RELOAD)
		{
			if (pdata.hud)
			{
				self.AddHudHideFlags(HIDEHUD_ALL);
				pdata.hud = false;
			}
			else
			{
				self.RemoveHudHideFlags(HIDEHUD_ALL);
				pdata.hud = true;
			}
		}

		pdata.buttons_last = buttons;
		return -1;
	},
		...
		if (!player.IsBotOfType(1337))
		{
			...
			if (pdata.loadoutpaint != null)
			{
				...
				foreach (slot, wpn_info in data)
					if (wpn_info.len())
						GiveWarpaintWeapon(player, wpn_info, pdata.loadoutpaint);
	...
	function OnGameEvent_player_say(params)
	{
		...
		case "g":
			...
			foreach (t in targets)
			{
				...
				GiveWarpaintWeapon(t, wpn_info, pdata.paint, pdata.wear, pdata.effect, pdata.killstreak, pdata.seed);
			}
		...
		case "seed":
		case "s":
			local err = HandleArgs(player, cmd, [{name="seed",type="integer",min_value=0,
												  required=true}]);
			if (err)
			{
				ClientPrint(player, 3, err);
		...
		case "lp":			
			...
			if (!paint)
			{
				pdata.loadoutpaint = null;
				ClientPrint(player, 3, "[CMD] Disabled loadout paint");
				break;
			}
			else
				pdata.loadoutpaint = paint;
		...
		case "thirdperson":
		case "tp":
			player.SetForcedTauntCam((pdata.thirdperson = !pdata.thirdperson).tointeger());
			ClientPrint(player, 3, format("[CMD] %s thirdperson", (pdata.thirdperson) ? "Enabled" : "Disabled"));
			break;
...
{
	Warpaints.ValidatePlayerData(player);
}

local script_ent = FindByName(null, "_warpaints_thinker");
if (!script_ent);
{
	script_ent = SpawnEntityFromTable("info_teleport_destination", {
		targetname = "_warpaints_thinker",
	});
}
script_ent.ValidateScriptScope();
script_ent.GetScriptScope().Think <- function() {
	foreach (player, data in Warpaints.PlayerData)
	{
		player.ValidateScriptScope();
		Warpaints.PlayerThink.call(player.GetScriptScope());
	}
	return -1;
};
AddThinkToEnt(script_ent, "Think");